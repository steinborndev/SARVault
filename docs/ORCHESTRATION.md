...
```mermaid
flowchart LR
  subgraph raw["Raw (extract + RDKit cheminfo + UMAP + enrichment)"]
    raw_activities["raw/activities"]
    raw_assays["raw/assays"]
    raw_compound_cheminfo["raw/compound_cheminfo"]
    raw_compound_embedding["raw/compound_embedding"]
  ...
  subgraph staging["Staging (dbt)"]
    ...
    staging_stg_compound_embedding["staging/stg_compound_embedding"]
  ...
  analytics_mart_compound_fingerprint --> analytics_mart_chemical_space
  analytics_mart_compound_selectivity --> analytics_mart_chemical_space
  marts_dim_compound --> analytics_mart_chemical_space
  staging_stg_compound_embedding --> analytics_mart_chemical_space
